...
class ActorCriticCNNGAE(nn.Module):
    def __init__(self, obs_shape, action_dim):
        super(ActorCriticCNNGAE, self).__init__()
...
class PPOCNNGAE:
    def __init__(self, obs_shape, action_dim, lr_actor, lr_critic, gamma, gae_lambda, K_epochs, eps_clip):
        self.gamma = gamma
        ...
        self.policy = ActorCriticCNNGAE(obs_shape, action_dim).to(device)
        ...
        self.policy_old = ActorCriticCNNGAE(obs_shape, action_dim).to(device)
    ...
    def update(self):
        ...
        for i in reversed(range(len(self.buffer.rewards))):
            ...
            if i == len(self.buffer.rewards) - 1 or self.buffer.is_terminals[i]:
                next_value = 0
            else:
                next_value = self.buffer.state_values[i+1].item()
                
            current_value = self.buffer.state_values[i].item()
            delta = self.buffer.rewards[i] + self.gamma * next_value * (1 - self.buffer.is_terminals[i]) - current_value
            gae = delta + self.gamma * self.gae_lambda * (1 - self.buffer.is_terminals[i]) * gae
        ...
        for _ in range(self.K_epochs):
            ...
            print(f"Ratio min: {ratios.min().item()}, max: {ratios.max().item()}")
            ...
            #print(f'policy loss {policy_loss}, value loss {value_loss}, entropy loss {entropy_loss}')
        ...
        print(f'Mean policy loss {torch.mean(policy_losses)}, Mean value loss {torch.mean(value_losses)}')
        self.policy_old.load_state_dict(self.policy.state_dict())
        self.buffer.clear()
